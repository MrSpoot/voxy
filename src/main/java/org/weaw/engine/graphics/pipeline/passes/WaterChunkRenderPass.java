...
/** Dedicated water path; generic transparency no longer pays for water deformation. */
public final class WaterChunkRenderPass extends AbstractChunkLayerPass {
    private double waterAnimationTime;

    public WaterChunkRenderPass(ChunkManager chunkManager) {
        super(chunkManager, "WaterChunkRenderPass", "/shaders/chunk-transparent.glsl");
    }
    ...
    protected LayerMeshData selectLayerMesh(ChunkUpload upload) {
        return upload.meshData().water();
    }

    @Override
    protected ChunkFaceArena selectArena(RenderContext context) {
        return context.getWaterChunkFaceArena();
    }
    ...
    protected void setLayerUniforms(RenderContext context, Shader shader) {
        ...
        shader.setUniform(
                "uWaterWavesEnabled",
                settings.areWavesEnabled()
                        && context.getAdaptiveGraphicsQuality().getLevel().waterWavesEnabled() ? 1 : 0
        );
    ...
    private static float distanceSquaredToCamera(ChunkRenderEntry entry, float cameraX, float cameraY, float cameraZ) {
        float dx = entry.originX() + Chunk.SIZE * 0.5f - cameraX;
        float dy = entry.originY() + Chunk.SIZE * 0.5f - cameraY;
        float dz = entry.originZ() + Chunk.SIZE * 0.5f - cameraZ;
